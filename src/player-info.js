...
AFRAME.registerComponent('player-info', {
  ...
  init: function () {
    ...
    this.boundSquadListener = this.applyNametagVisibility.bind(this);
    if (typeof window !== 'undefined') {
      window.addEventListener('nusa:squad-context-changed', this.boundSquadListener);
    }
  },
  ...
  update: function (oldData) {
    ...
    // Re-query nametag in case it was created after init
    if (!this.nametag) {
      this.nametag = this.el.querySelector('.nametag');
    }
    if (this.nametag && this.data.name) {
      this.nametag.setAttribute('value', this.data.name);
    }
    this.applyNametagVisibility();
  ...
  applyNametagVisibility: function () {
    if (!this.nametag) {
      this.nametag = this.el.querySelector('.nametag');
    }
    if (!this.nametag) return;
    const viewerSquad =
      typeof window !== 'undefined' ? window.__activeSquadId ?? null : null;
    const targetSquad = this.data.squadId || null;
    const shouldShow =
      this.el.id === 'rig' || !viewerSquad || !targetSquad || viewerSquad === targetSquad;
    this.nametag.setAttribute('visible', shouldShow);
  },

  remove: function () {
    if (typeof window !== 'undefined' && this.boundSquadListener) {
      window.removeEventListener('nusa:squad-context-changed', this.boundSquadListener);
    }
  },
  ...
  events: {
    ...
    'model-loaded': function (evt) {
      // Update nametag after model loads
      if (!this.nametag) {
        this.nametag = this.el.querySelector('.nametag');
      }
      if (this.nametag && this.data.name) {
      ...
      // Adjust avatar model position to make feet touch ground properly
      // Balanced adjustment: lower if floating, raise if too low
      queueMicrotask(() => {
        ...
        setTimeout(() => {
          ...
          // Adjust if avatar is floating (minY > 0.25) - use moderate adjustment
          if (minY > 0.25) {
            const currentPos = this.avatarEl.getAttribute('position') || { x: 0, y: 0, z: 0 };
            // Moderate adjustment: move down by 60% of the excess, keep 0.1 unit gap
            const excess = minY - 0.1;
            const adjustment = excess * 0.6;
            const newY = currentPos.y - adjustment;
            
            this.avatarEl.setAttribute('position', {
              x: currentPos.x,
              y: newY,
              z: currentPos.z
            });
            console.log('[PlayerInfo] Adjusted avatar down - minY:', minY, 'Excess:', excess, 'Adjustment:', adjustment, 'New Y:', newY);
          } else if (minY < -0.05) {
            // Avatar is too low (underground or sinking), move it up
            const currentPos = this.avatarEl.getAttribute('position') || { x: 0, y: 0, z: 0 };
            // Move up by the negative minY plus gap to keep feet on ground
            const adjustment = -minY + 0.1;
            const newY = currentPos.y + adjustment;
            ...
            console.log('[PlayerInfo] Adjusted avatar up (was too low/sinking) - minY:', minY, 'Adjustment:', adjustment, 'New Y:', newY);
          } else {
            // Avatar is at reasonable position (minY between -0.05 and 0.25), don't adjust
            console.log('[PlayerInfo] Avatar position OK, no adjustment needed - minY:', minY);
          }
        }, 300);
      });
