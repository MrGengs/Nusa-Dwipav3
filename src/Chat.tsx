...
import { getClientsInSquad } from './presence';
...
const [currentSquadId, setCurrentSquadId] = createSignal<string | null>(null);

const syncCurrentSquad = (squadId: string | null) => {
  setCurrentSquadId(squadId);
  setMessages('entries', []);
  setMessages('firstUnreadKey', null);
};

const resolveSquadId = () => {
  const session = (window as any).roomSession;
  return session?.squadId ?? null;
};

if (typeof window !== 'undefined') {
  setCurrentSquadId(resolveSquadId());
  window.addEventListener('room:squad-changed', (event: Event) => {
    const detail = (event as CustomEvent<{ squadId: string | null }>).detail;
    const squadId = detail?.squadId ?? null;
    if (squadId !== currentSquadId()) {
      syncCurrentSquad(squadId);
    }
  });
}

const addToChat = (entry: LogEntry) => {
  const squadId = currentSquadId();
...
const sendMessage = (text: string, name: string) => {
  const squadId = currentSquadId();
  if (!squadId) {
    console.warn('[Chat] Unable to send message without joining a squad.');
    return;
  }
  const entry: ChatMessage = { type: 'chat', text: text, name: name, fromClientId: NAF.clientId, squadId };
  addToChat(entry);
  const recipients = getClientsInSquad(squadId).filter((clientId) => clientId !== NAF.clientId);
  if (typeof NAF.connection.sendDataGuaranteed !== 'function') {
    console.warn('[Chat] Targeted messaging is not supported by this adapter.');
    return;
  }
  recipients.forEach((clientId) => {
    NAF.connection.sendDataGuaranteed?.(clientId, 'chat', entry);
  });
};
...
export const ChatPanel: Component<ChatPanelProps> = (props) => {
  const sendMessageAndResetInput = (e: Event) => {
    e.preventDefault();
    if (input.value.length > 0 && currentSquadId()) {
    ...
    <Show when={showChatPanel()}>
      <div class="bg-panel absolute bottom-16 left-2 right-2 top-16 z-10 flex max-w-full flex-col justify-between rounded-lg p-4 shadow-lg ring-1 ring-black ring-opacity-5 sm:left-auto sm:w-screen sm:max-w-sm">
        ...
        <div class="flex grow flex-col overflow-y-auto">
          <Show when={!currentSquadId()}>
            <div class="rounded-lg bg-amber-100 p-3 text-sm text-amber-700">
              Chat hanya tersedia setelah kamu bergabung ke salah satu squad.
            </div>
          </Show>
        ...
        <form class="relative flex" onSubmit={sendMessageAndResetInput}>
          <textarea
            ...
            placeholder={currentSquadId() ? 'Pesan untuk squad kamu' : 'Gabung ke squad untuk mulai chat'}
            disabled={!currentSquadId()}
            onInput={(e) => {
